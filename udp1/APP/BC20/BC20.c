...
#define SERVERIP "81.68.239.40 "
//#define SERVERIP "101.200.212.234  "
//#define SERVERIP "47.109.35.113"
#define SERVERPORT 1006
...
void BC20_SenddataHEX(uint8_t *len,uint8_t *data)
{
    int errcount=0;
    memset(atstr,0,BUFLEN);
    sprintf(atstr,"AT+QISENDEX=%d,%s,%s\r\n",socketnum,len,data);
    Uart2_SendStr(atstr);//发送0 socketIP和端口后面跟对应数据长度以及数据
    delay_ms(300);
    strx=strstr((const char*)buf_uart2.buf,(const char*)"OK");//返回OK
    while(strx==NULL)
    ...
    strx=strstr((const char*)buf_uart2.buf,(const char*)"socket_t is NULL");
    if(strx)
    {
